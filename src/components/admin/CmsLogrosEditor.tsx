...
type EditState = Omit<Logro, "id">;

const STORAGE_BUCKET = "cms-imagenes";

const ICONOS = [
  { key: "helmet", label: "🛡️ Casco (obras / infraestructura)" },
  { key: "ship", label: "🚢 Barco (puerto / marítimo)" },
  { key: "water", label: "💧 Agua (saneamiento)" },
  { key: "fire", label: "🔥 Fuego (energía / GLP)" },
  { key: "heart", label: "❤️ Corazón (salud / social)" },
];
...
export default function CmsLogrosEditor() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const rows = (data as Logro[]) || [];
      setLogros(rows);
      const initial: Record<string, EditState> = {};
      rows.forEach((l) => {
        initial[l.id] = {
          orden: l.orden,
          icon_key: l.icon_key || "helmet",
          title: l.title || "",
          body: l.body || "",
          image: l.image || "",
  ...
  const subirImagen = async (
    ...
    try {
      ...
      const path = `logros/${Date.now()}_${safe}`;
  ...
  const eliminar = async (id: string) => {
    ...
    try {
      const supabase = getSupabase();
      const { error } = await supabase.from("cms_logros").delete().eq("id", id);
      if (error) throw error;
      await fetchData();
    } catch (err) {
      alert("Error: " + (err instanceof Error ? err.message : ""));
  ...
  const agregar = async () => {
    ...
    try {
      const supabase = getSupabase();
      const maxOrden = logros.reduce((a, l) => Math.max(a, l.orden), 0);
      const { error } = await supabase.from("cms_logros").insert({
        ...nueva,
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 max-w-2xl">
            Sección "Por Amor y Justicia con Manabí". Cards con ícono, título y
            descripción.
          </p>
          <div className="mt-3">
            <span className="px-3 py-1 bg-[#EAE84B] text-[#6F2C91] font-bold text-xs rounded-lg">
              {logros.length} item{logros.length === 1 ? "" : "s"}
            </span>
            {loading && (
              <span className="ml-2 text-[#6F2C91] text-xs font-bold animate-pulse">
                Cargando...
              </span>
            )}
          </div>
        ...
        >
          {showAdd ? "Cancelar" : "+ Agregar logro"}
        </button>
      </div>
      ...
      {showAdd && (
        <div className="mb-8 p-6 rounded-[2rem] bg-white shadow-sm border-2 border-[#6F2C91]/30">
          ...
          <div className="flex justify-end gap-3 mt-4">
            ...
            >
              {savingNueva ? "Guardando..." : "Guardar logro"}
            </button>
          </div>
        </div>
      )}
...
function LogroForm({ estado, onChange, uploading, onUpload }: LogroFormProps) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[180px,1fr] gap-6">
      <div className="space-y-3">
        <div className="rounded-xl overflow-hidden bg-gray-100 aspect-square flex items-center justify-center">
          {estado.image ? (
            <img src={estado.image} alt="" className="w-full h-full object-cover" />
          ) : (
        ...
        <label className="block">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (f) onUpload(f, (url) => set({ image: url }));
              e.target.value = "";
            }}
          />
        </label>
